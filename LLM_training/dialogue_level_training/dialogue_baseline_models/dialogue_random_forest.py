# RANDOM FOREST DIALOGUE LEVEL VERSION
...
# File path - UPDATE THIS to point to your dialogue-level Excel file
...
# Create Random Forest pipelines optimized for dialogue-level text
print("\n===== CREATING RANDOM FOREST MODELS =====")

pipelines = {
    'RF_Balanced_100_Dialogue': Pipeline([
        ('tfidf', TfidfVectorizer(
            max_features=15000,  # Increased for dialogue complexity
            ngram_range=(1, 3),  # Include trigrams for dialogue patterns
            min_df=2,
            max_df=0.90,  # Lower for dialogue-specific terms
        ...
        ('classifier', RandomForestClassifier(
            n_estimators=100,
            max_depth=15,  # Increased depth for dialogue complexity
            min_samples_split=5,
            min_samples_leaf=2,
            max_features='sqrt',  # Good for high-dimensional dialogue features
            random_state=42,
    ...
    'RF_Balanced_200_Dialogue': Pipeline([
        ('tfidf', TfidfVectorizer(
            max_features=15000,
            ngram_range=(1, 3),
            min_df=2,
        ...
        ('classifier', RandomForestClassifier(
            n_estimators=200,
            max_depth=20,  # Deeper trees for dialogue patterns
            min_samples_split=5,
            min_samples_leaf=2,
            max_features='sqrt',
            random_state=42,
    ...
    'RF_Conservative_Dialogue': Pipeline([
        ('tfidf', TfidfVectorizer(
            max_features=20000,  # Even more features
            ngram_range=(1, 2),  # Conservative n-grams to reduce overfitting
            min_df=3,  # Higher min_df for dialogue stability
            max_df=0.85,
            strip_accents='unicode',
            lowercase=True,
            stop_words='english',
...
if best_model['top_features']:
    print(f"\nTop 10 Most Predictive Features for Dialogue Manipulation:")
...
def predict_dialogue_manipulation(dialogue_text, threshold=None):
    """Predict manipulation using the best Random Forest model for dialogues"""
...
test_dialogue_samples = [
    ...
    "Person A: If you really loved me, you would do this for me. Person B: That's not how love works. Person A: Yes it is! You're being selfish. Person B: I need some time to think about this."
]
...
print(f"\n{'=' * 70}")
print("RANDOM FOREST MODEL COMPARISON - DIALOGUE LEVEL")
print(f"{'=' * 70}")
...
# Random Forest specific insights for dialogue-level
print(f"\n{'=' * 70}")
print("DIALOGUE-LEVEL RANDOM FOREST INSIGHTS")
print(f"{'=' * 70}")

print("Random Forest Advantages for Dialogue Analysis:")
print("  - Captures complex conversational patterns and interactions")
print("  - Feature importance reveals key manipulative words/phrases")
print("  - Handles variable dialogue lengths well")
print("  - Can detect non-linear combinations of features")
print("  - Robust to noise in conversational text")

print(f"\nDialogue-Specific Model Complexity:")
rf_params = best_model['pipeline'].named_steps['classifier']
tfidf_params = best_model['pipeline'].named_steps['tfidf']
print(f"  - Best model: {best_model_name}")
print(f"  - Number of trees: {rf_params.n_estimators}")
print(f"  - Max depth: {rf_params.max_depth}")
print(f"  - Max features (TF-IDF): {tfidf_params.max_features}")
print(f"  - N-gram range: {tfidf_params.ngram_range}")
print(f"  - Training time: {best_model['training_time']:.2f}s")

# Dialogue vs Sentence Analysis Notes
print(f"\nDialogue vs Sentence-Level Considerations:")
print("  - Dialogues provide more context but also more noise")
print("  - Feature space is larger due to conversational patterns")
print("  - May capture turn-taking and power dynamics")
print("  - Could be more sensitive to dialogue structure and flow")

# Optional: Save the best model
try:
    model_path = "best_dialogue_random_forest_model.pkl"
...
print(f"\n{'=' * 70}")
print("DIALOGUE-LEVEL RANDOM FOREST ANALYSIS COMPLETE")
print(f"{'=' * 70}")
print(f"Best Random Forest: {best_model_name} (F1: {best_model['test_f1']:.4f})")
print("Compare this with your:")
print("  - Sentence-level Random Forest results")
print("  - Dialogue-level Logistic Regression results")
print("  - Dialogue-level BERT results")
print("\nCheck the feature importance to understand what dialogue patterns")
print("are most predictive of manipulation!")
...
# Enhanced manipulation definition for dialogues
...
Focus on the overall conversational dynamic and psychological impact, not just individual sentences.
"""


def select_dialogue_training_examples(df_train, k_manip=2, k_nonmanip=2, random_state=None):
    """
    Select actual training dialogue examples for few-shot prompting.

    Args:
        df_train: Training dataframe
        k_manip: Number of manipulative dialogue examples
        k_nonmanip: Number of non-manipulative dialogue examples
    ...
    if len(manip_indices) < k_manip:
        print(f"Warning: Only {len(manip_indices)} manipulative training dialogues available")
    ...
    if len(nonmanip_indices) < k_nonmanip:
        print(f"Warning: Only {len(nonmanip_indices)} non-manipulative training dialogues available")
...
def create_dialogue_fewshot_prompt(manip_examples, nonmanip_examples, test_dialogue):
    """
    Create a few-shot prompt with actual training dialogue examples.
    """

    examples_text = "Here are some examples from this dataset:\n\n"

    # Add manipulative examples
    examples_text += "MANIPULATIVE DIALOGUE EXAMPLES:\n"
    for idx, row in manip_examples.iterrows():
        dialogue = str(row['dialogue']).strip()
        # Truncate very long dialogues for prompt efficiency
        if len(dialogue) > 300:
            dialogue = dialogue[:300] + "..."
        examples_text += f"Example {idx + 1}:\n{dialogue}\n→ MANIPULATIVE\n\n"

    examples_text += "NON-MANIPULATIVE DIALOGUE EXAMPLES:\n"
    ...
    examples_text += f"Now analyze this dialogue:\n{test_dialogue}\n\nBased on the patterns shown in the examples above, is this dialogue manipulative or non-manipulative? Consider the overall conversational dynamic and psychological impact."

    return examples_text


def classify_dialogue_with_fewshot(dialogue_text, manip_examples, nonmanip_examples):
    """
    Classify a dialogue using few-shot prompting with training examples.
    """
    schema = {
        "type": "object",
        "properties": {
            "is_manipulative": {
                "type": "boolean",
                "description": "Whether the dialogue contains mental manipulation"
            },
            "confidence": {
                "type": "number",
                "minimum": 0,
    ...
    system_prompt = f"""You are an expert at identifying mental manipulation in conversational dialogues.

{MANIPULATION_DEFINITION}

CONTEXT: These are dialogues from movie scripts featuring interpersonal conversations. 
Focus on the psychological impact and intent behind the words, not just their surface meaning.
Consider the conversational flow and how speakers may be trying to influence each other.

Based on the training examples provided, analyze whether the given dialogue contains mental manipulation.
Pay attention to the patterns you see in the provided examples from this specific dataset.

Provide your analysis in JSON format with:
1. is_manipulative: true/false
2. confidence: 0-1 score
3. reasoning: brief explanation focusing on conversational dynamics

Consider:
1. What is each speaker trying to achieve in this conversation?
2. Are there emotional pressure tactics, deception, or reality distortion?
3. Does anyone try to control or influence the other's psychological state?
4. Look at the overall conversational dynamic, not just individual sentences"""

    user_prompt = create_dialogue_fewshot_prompt(manip_examples, nonmanip_examples, dialogue_text)
    ...
    try:
        response = client.chat.completions.create(
            ...
            max_tokens=1200,  # Increased for dialogue analysis
...
def run_improved_dialogue_classification(excel_file, dialogue_column, label_column, output_file,
                                         n_test_per_class=25, k_manip=2, k_nonmanip=2,
                                         train_ratio=0.7, batch_size=10):
    """
    Run improved dialogue classification with few-shot training examples.

    Args:
        excel_file (str): Path to Excel file
        dialogue_column (str): Name of column containing dialogues
        label_column (str): Name of column containing true labels
        output_file (str): Path to save JSON results
        n_test_per_class (int): Number of test dialogues per class
        ...
        batch_size (int): Save progress every N dialogues
    """

    results = []

    try:
        # Read dialogues from Excel file
        df = pd.read_excel(excel_file)
        print(f"Loaded {len(df)} dialogues from Excel file")
        print(f"Available columns: {list(df.columns)}")

        # Clean data
        df = df.dropna(subset=[dialogue_column, label_column])
        df[dialogue_column] = df[dialogue_column].astype(str).str.strip()

        # Remove empty dialogues
        df = df[df[dialogue_column] != '']

        print(f"After cleaning: {len(df)} dialogues")
        ...
        print(f"Training set: {len(df_train)} dialogues")
        print(f"Available for testing: {len(df_test_full)} dialogues")
        ...
        manip_examples, nonmanip_examples = select_dialogue_training_examples(
        ...
        print("Manipulative dialogue examples:")
        for idx, row in manip_examples.iterrows():
            dialogue_preview = str(row[dialogue_column])[:150] + "..." if len(str(row[dialogue_column])) > 150 else str(
                row[dialogue_column])
            print(f"  - {dialogue_preview}")
        print("Non-manipulative dialogue examples:")
        for idx, row in nonmanip_examples.iterrows():
            dialogue_preview = str(row[dialogue_column])[:150] + "..." if len(str(row[dialogue_column])) > 150 else str(
                row[dialogue_column])
            print(f"  - {dialogue_preview}")

        print(f"\nProcessing {len(df_test)} test dialogues...")

        # Process each test dialogue
        for i, row in df_test.iterrows():
            dialogue_text = str(row[dialogue_column])
            true_label = row[label_column]

            result = classify_dialogue_with_fewshot(dialogue_text, manip_examples, nonmanip_examples)

            # Add metadata
            result['dialogue_id'] = i + 1
            result['dialogue_text'] = dialogue_text
            result['true_label'] = bool(true_label)
            results.append(result)

            dialogue_preview = dialogue_text[:50] + "..." if len(dialogue_text) > 50 else dialogue_text
            print(f"Processed dialogue {i + 1}/{len(df_test)}: {dialogue_preview}")

            # Save progress incrementally
            if (i + 1) % batch_size == 0:
                with open(output_file, 'w', encoding='utf-8') as f:
                    json.dump(results, f, indent=2, ensure_ascii=False)
                print(f"Saved progress: {i + 1} dialogues completed")
        ...
        print(f"\nDialogue Classification Complete!")
...
if __name__ == "__main__":
    ...
    # Using same test size (25 per class = 50 total) for cost control and comparison
    run_improved_dialogue_classification(
        excel_file=excel_file,
        dialogue_column=dialogue_column,
        label_column=label_column,
        output_file='dialogue_manipulation_results_fewshot_balanced.json',
        n_test_per_class=25,  # Same as your original for comparison
        k_manip=2,  # Number of manipulative training examples
        k_nonmanip=2,  # Number of non-manipulative training examples
        train_ratio=0.7,  # 70% for training examples, 30% for testing
        batch_size=10  # Smaller batch size for dialogues (they're longer)
    )